import React, { ReactNode, useCallback, useEffect, useState, Component, useRef, useMemo } from 'react';
...
import { Filter, Opacity, PrintRounded, WarningAmberSharp } from '@mui/icons-material';
...
import AcceptBeforeCountdown from '@/components/order/AcceptBeforeCountdown';
import { PiPrinter, PiPrinterBold, PiPrinterDuotone } from 'react-icons/pi';
import { FiPrinter } from 'react-icons/fi';
import cancelTempOrderAPI from '@/services/order/API/cancelTempOrder';
...
const OrderTable: React.FC = () => {
  ...
  const [isLoadingViewOrder, setIsLoadingViewOrder] = useState(false);
  const [isLoadingDrawerChange, setIsLoadingDrawerChange] = useState(false);
  ...
  const [isAcceptTimeOut, setIsAcceptTimeOut] = useState([]);
  const [openDrawerId, setOpenDrawerId] = useState(-1);
  const [sheduledWaitingOrdersCount, setSheduledWaitingOrdersCount] = useState(0);

  const [currentOrderCancellable, setCurrentOrderCancellable] = useState(false);

  const [backendCallLoading, setBackendCallLoading] = useState(false);
  const buttonRef = useRef<HTMLButtonElement>(null);
  const activeTabRef = useRef(activeTab);
  const currentOpenDrawerId = useRef(openDrawerId);
  ...
  useEffect(() => {
    currentOpenDrawerId.current = openDrawerId;
  }, [openDrawerId]);
  ...
  // Define rowsPerPageOptions based on totalNumberOfOrders
  const rowsPerPageOptions =
    totalNumberOfOrders === 0 ? [5, 10, 25] : [5, 10, 25, { label: 'All', value: totalNumberOfOrders }];
  ...
  const toggleDrawer1 = (index) => {
    setOpenDrawerId(openDrawerId === index ? -1 : index);
  };
  ...
  const { mutate: cancelTempOrderCall } = cancelTempOrderAPI();
  ...
  useEffect(() => {
    ...
    socket.on('new_order', async (data) => {
      try {
        await Promise.all([fetchOrders(), fetchOrderCount()]);
        showAlert({ message: 'New Order Received!!!', type: 'success' });
      } catch (error) {
        console.error('Error fetching orders or order count:', error);
      }
    });
          ...
          onSuccess: (data) => {
            ...
            tempData.forEach((tempOrder: any) => {
              ...
              const order: Order = {
                ...
                source_name: tempOrder.source_name,
                ...
                accept_before: new Date(tempOrder.accept_before)
              };
  ...
  const checkOrderToCancel = () => {
      const currentTime = new Date();
      orders.forEach((order: { accept_before: string | number | Date; source_name: any; temp_order: number | null; }) => {
          const acceptBeforeTime = new Date(order.accept_before);
          console.log("oo", order)

          if (currentTime > acceptBeforeTime && order.source_name == 'graun foods') {
            cancelTempOrder(order.temp_order, 'Restaurant Did not Accept the Order!', 'other');
          }
      });
  };
        ...
        },
        {
          onSuccess: (data) => {
            ...
            const orderObject = {
              ...
              contact_access_code: dmsOrderObject.customer?.contact_access_code || 'N/A',
              accept_before: new Date(tempData.accept_before)
            };
            setOrderDetails(orderObject);
            setOrderSource(tempData.source_name);
            setOpen(true);
            (tempData.source_name == 'uber eats' || tempData.source_name == 'graun foods') ? setCurrentOrderCancellable(true) : setCurrentOrderCancellable(false);
            setIsLoadingViewOrder(false)
          },
          onError: (error) => {
            // Handle error
            console.error('Error:', error);
      ...
      getOrderDetailsCall(
        ...
        },
        {
          onSuccess: (data) => {
            ...
            (data.order_source.order_source == 'uber eats' || data.order_source.order_source == 'graun foods')
              ? setCurrentOrderCancellable(true)
              : setCurrentOrderCancellable(false);
            setOpen(true);
            setIsLoadingViewOrder(false)
          },
          onError: (error) => {
  ...
  const acceptOrder = async (tempOrderId: number | null) => {
    ...
    if (activeTab == 'order_waiting' || activeTab == 'all_orders') {
      ...
      acceptOrderCall(
        ...
        },
        {
          ...
          onError: (error: any) => {
            // Handle error
            console.error('Error:', error);
            showAlert({ message: 'Unable to Accept the Order!', type: 'error' });
            // alert(error.message);
            setSelectedMenuValue('');
            setBackendCallLoading(false);
          },
  ...
  const rejectOrder = async (tempOrderId: number | null, rejectNote: any, rejectReason: any) => {
    ...
    if (
      (rejectReason == null || rejectReason == '' || rejectNote == null || rejectNote == '') &&
    ...
    if (
      activeTab == 'order_waiting' ||
      (activeTab == 'all_orders' && orderDetails.order_source.order_source != 'graun foods')
    ) {
      rejectOrderCall(
        {
          temp_order_id: tempOrderId,
  ...
  const cancelOrder = async (orderId: number | null, cancelNote: any, cancelReason: any) => {
    ...
    if (cancelNote == null || cancelNote == '' || cancelReason == null || cancelReason == '') {
      // alert('Provide a valid Rejection Reason and a note!');
      showAlert({ message: 'Provide a valid Rejection Reason and a note!', type: 'error' });
      return;
    }
  ...
  const cancelTempOrder = async (orderId: number | null, cancelNote: any, cancelReason: any) => {
    if (orderId == null) {
      // alert('Unable to accept order with null  order id!');
      showAlert({ message: 'Unable to cancel order with null order id!', type: 'error' });
      return;
  ...
  useEffect(() => {
    ...
    console.log(sheduledWaitingOrdersCount)
    if (orderWaitingOrderCount > 0 || sheduledWaitingOrdersCount > 0) {
      audio.loop = true;
    ...
    return () => {
      audio.pause(); // Clean up by pausing the audio when the component unmounts or orderWaitingOrderCount changes
    };
  }, [orderWaitingOrderCount, sheduledWaitingOrdersCount]);
  ...
  useEffect( () => {
    ...
    fetchOrderCount();
      ...
      const intervalId = setInterval(() => {
        if (currentOpenDrawerId.current == -1) {
          fetchOrders();
          fetchOrderCount();
        }
      }, 30000);
  ...
  useEffect( () => {
    ...
    const payload = {
      pageNumber: 1,
      recordsPerPage: 40,
      order_status: 'waiting for prep',
      order_type_id: 2 // sheduled
    }
    getOrdersCall(payload, {
      onSuccess: (data) => {
        setSheduledWaitingOrdersCount(data.totalCount);
      },
      onError: (error) => {
        // Handle error
        console.error('Error:', error);
  ...
  const closeCancelPopup = () => {
    setResetButton((val) => !val);
    setOpenCancelPop(false);
  };
  ...
  const tabs = [
    { id: 'all_orders', label: 'All Orders', count: formatCount(allOrderCount) },
    { id: 'order_waiting', label: 'Order waiting', count: formatCount(orderWaitingOrderCount) },
    { id: 'waiting_for_prep', label: 'Waiting for Prep', count: formatCount(waitingForPrepOrderCount) },
    { id: 'in_prep', label: 'In Prep', count: formatCount(inPrepOrderCount) },
    { id: 'ready_for_collection', label: 'Ready for collection', count: formatCount(readyForCollectionOrderCount) },
    { id: 'rejected', label: 'Rejected', count: formatCount(rejectedOrderCount) },
    { id: 'cancelled', label: 'Cancelled', count: formatCount(cancelledOrderCount) },
  ];
  ...
  const renderTabs = () => {
    return tabs.map((tab) => (
      <Grid
        item
        xs={1.5} // Adjust the width to ensure proper spacing
        key={tab.id}
        style={{
  ...
  const updateOrderStatusButton = useMemo(() => {
    if (orderDetails.order_status?.order_status ) {
      /* waiting_for_prep */
      if (orderDetails.order_status.order_status == 'waiting for prep') {
        setSelectedMenuValue('in prep');
        return 'In Prep';
      }
      /* in_prep */
      if (orderDetails.order_status.order_status == 'in prep') {
        if (orderDetails.delivery_mode?.delivery_mode == 'restaurant') {
          setSelectedMenuValue('ready for deliver');
          return 'Ready for Delivery';
        } else {
          setSelectedMenuValue('ready for collection');
          return 'Ready for Collection';
        }
      }
      /* ready_for_collection */
      if (orderDetails.order_status?.order_status == 'ready for collection') {
        setSelectedMenuValue('collected');
        return 'Collected';
      }
    }
  }, [orderDetails.order_status?.order_status, resetButton, selectedMenuValue]);

  useEffect(() => {
    if (openDrawerId !== null) {
      // Using a timeout to ensure the drawer has fully opened and the button is rendered
      const timer = setTimeout(() => {
        if (buttonRef.current) {
          buttonRef.current.classList.add('focus-transition');
      ...
      return () => {
        clearTimeout(timer);
        if (buttonRef.current) {
          buttonRef.current.classList.remove('focus-transition');
        }
      };
    }
  }, [openDrawerId]);
  ...
  const DrawerList =
    orderDetails !== null && Object.keys(orderDetails).length > 0 ? (
      ...
      >
        <div style={{ display: 'flex', marginBottom: 10, alignItems:'center' }}>
          <div style={{ display: 'flex', width:'80%'}}>
            ...
            <div>
              {(orderDetails.order_status?.order_status || orderDetails.order_status) == 'pending'? 
              <>
                {orderDetails.accept_before ? <AcceptBeforeCountdown acceptBefore={orderDetails.accept_before} setIsAcceptTimeOut={setIsAcceptTimeOut} orderId={orderDetails.order_id} cancelOrder={orderDetails.source_name == 'graun foods' ? cancelOrder : () => {}} /> : <></>}
              </>
              :<></>}
            </div>
          </div>
          ...
          <div style={{ marginLeft: 'auto', fontSize: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 5 }}>
              ...
              <p style={{ fontSize: 14, textTransform: 'capitalize' }}>
                {orderDetails.order_type?.order_type == 'scheduled'? 
                                          <div
                                          style={{
                                            color: 'red',
                                            marginLeft: '10px',
        ...
        >
          <TableContainer sx={{}}>
            <Table sx={{ minWidth: 200 }} aria-label="customized table">
              <TableHead>
                <TableRow>
                  <TableCell className="customTableCell" style={{fontSize:15}}>Items</TableCell>
                  <TableCell className="customTableCell" style={{fontSize:15}}>Qty</TableCell>
                  <TableCell className="customTableCell" style={{fontSize:15}}>Unit Price</TableCell>
                  <TableCell className="customTableCell" style={{fontSize:15}}>Price</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {orderDetails.items.map((item: Item, index: React.Key | null | undefined) => (
                  <>
                    <StyledTableRow key={index}>
                      <StyledTableCell className="customTableRow" style={{fontSize:15}}>{item.item_name}</StyledTableCell>
                      <StyledTableCell className="customTableRow" style={{fontSize:15}}>{item.quantity}</StyledTableCell>
                      <StyledTableCell className="customTableRow" style={{fontSize:15}}>
                ...
                <StyledTableRow>
                  <TableCell style={{fontSize:15}}>Subtotal</TableCell>
                  <TableCell></TableCell>
                  <TableCell></TableCell>
                  <TableCell style={{fontSize:15}}>£ {(parseFloat(orderDetails.subtotal_amount) / 100).toFixed(2)}</TableCell>
                </StyledTableRow>
              </TableBody>
            </Table>
          </TableContainer>
          ...
          {orderDetails.bag_fee ? (
            ...
            >
              ...
              >
                £ {(parseFloat(orderDetails?.bag_fee || '0.00') / 100).toFixed(2)}
              </p>
            </div>
          ) : (
            <></>
          ...
          {/* {orderSource == 'deliveroo' ? (
            ...
            >
              ...
              >
                £ {(parseFloat(orderDetails?.tax || '0.00') / 100).toFixed(2)}
              </p>
            </div>
          )} */}
          ...
          >
            <p>
              Order Total <span style={{ fontSize: 12 }}>(Inc. VAT): </span>
            </p>
            <p
              style={{
                marginLeft: '5px',
            ...
            >
              £ {(parseFloat(orderDetails.total_order_amount) / 100).toFixed(2)}
            </p>
          </div>
        </div>
        ...
        {(orderDetails.order_status?.order_status || orderDetails.order_status) == 'pending' ? (
          <>
          {!isAcceptTimeOut.includes(orderDetails.order_id) ? 
            <div
              style={{
                display: 'flex',
                position: 'relative',
            ...
            >
              ...
              <RejectOrderModal
                ...
                setBackendCallLoading={setBackendCallLoading}
                backendCallLoading={backendCallLoading}
              />
              ...
              >
                {!backendCallLoading ? 'Accept' : <Dots />}
              </button>
            </div>
            : <>
            <div
            ...
            >
              ...
              >
                <WarningAmberSharp /> &nbsp; Accept Time Out !!!
              </div>
            </div>
            </>}
          </>
          ...
          >
            {currentOrderCancellable && activeTab !== 'cancelled' && (
              <button
                style={{
                  backgroundColor: 'red',
                  color: 'white',
                ...
                disabled={backendCallLoading}
                onClick={() => {
                  setSelectedMenuValue('cancel');
                  setTimeout(() => {
                    functionOpenCancelPopup();
                  }, 800);
                }}
              >
                {!backendCallLoading ? 'Cancel' : <Dots />}
              </button>
            )}
            <button
              style={{
            ...
            >
              {!backendCallLoading ? (
                !updateOrderStatusButton && selectedMenuValue == 'cancel' && currentOrderCancellable ? (
                  'Cancel Order'
                ) : (
  ...
  useEffect(() => {
    // Reset page to 0 if totalNumberOfOrders changes and is less than current page * rowsPerPage
    if (page * rowsPerPage >= totalNumberOfOrders) {
      setPage(0);
    }
  }, [totalNumberOfOrders, rowsPerPage, page]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            {validationError && <p style={{ color: 'red', marginLeft: '14px', fontSize: '14px' }}>{validationError}</p>}
            ...
            >
              ...
              >
                <option value="">Select</option>
                <option value="deliveroo">By Deliveroo</option>
                <option value="uber">By Uber</option>
      ...
      >
        <Grid container spacing={0} display={'flex'} justifyContent={'space-around'}>
        {renderTabs()}
        </Grid>
      </Box>
                    ...
                    return (
                      <StyledTableRow key={index}>
                        ...
                        >
                          ...
                          {/* { order.accept_before ? 
                          <AcceptBeforeCountdown acceptBefore={order.accept_before} />
                          : <></> } */}
                        </StyledTableCell>
                        <StyledTableCell
                          sx={{
                        ...
                        >
                          ...
                          >
                            £ {(parseFloat(order.total_order_amount) / 100).toFixed(2)}
                          </div>
                        ...
                        <StyledTableCell>
                          ...
                          >
                            {isLoadingViewOrder && openDrawerId === index ? <Dots /> : 'View Order' }
                          </button>
                          {!isLoadingViewOrder ?
                          <Drawer
                            ...
                            transitionDuration={{ enter: 300, exit: 300 }} // Adjust the transition duration if needed
                          >
                            {!isLoadingViewOrder ? DrawerList :<Dots /> }
                            {/* {DrawerList} */}
                          </Drawer>
                          :<></> }
                        </StyledTableCell>
