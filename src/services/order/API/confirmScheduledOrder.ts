...
export const postApiRequest = async (payload: any): Promise<ResponseDataType> => {
  const { CONFIRM_SCHEDULED_ORDER } = ORDER_API_URLS;
  return requests.orderPost(CONFIRM_SCHEDULED_ORDER, payload);
};
// NOTE: USE handler when you want to use logic from within the component. else defile the onSuccess, onError etc here
const confirmScheduledOrderAPI = (handler?: HandlerType) => {
...
export default confirmScheduledOrderAPI;
