...
export const postApiRequest = async (payload: any): Promise<ResponseDataType> => {
  const { CHECK_UBER_INTEGRATION } = RESTAURANT_API_URLS;
  return requests.orderPost(CHECK_UBER_INTEGRATION, payload);
};
// NOTE: USE handler when you want to use logic from within the component. else defile the onSuccess, onError etc here
const checkUberIntegrationAPI = (handler?: HandlerType) => {
...
export default checkUberIntegrationAPI;
