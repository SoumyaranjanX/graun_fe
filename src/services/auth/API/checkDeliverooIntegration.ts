...
export const postApiRequest = async (payload: any): Promise<ResponseDataType> => {
  const { CHECK_DELIVEROO_INTEGRATION } = RESTAURANT_API_URLS;
  return requests.restaurantPost(CHECK_DELIVEROO_INTEGRATION, payload);
};
// NOTE: USE handler when you want to use logic from within the component. else defile the onSuccess, onError etc here
const checkDeliverooIntegrationAPI = (handler?: HandlerType) => {
...
export default checkDeliverooIntegrationAPI;
