...
export const postApiRequest = async (payload: any): Promise<ResponseDataType> => {
  const { SET_RESTAURANT_PAYMENT_ONBOARD } = DISPATCH_API_URLS;
  return requests.dispatchPost(SET_RESTAURANT_PAYMENT_ONBOARD, payload);
};
// NOTE: USE handler when you want to use logic from within the component. else defile the onSuccess, onError etc here
const setRestaurantPaymentOnboard = (handler?: HandlerType) => {
  ...
  return useMutation(
    ...
    },
    {
      ...
      onSettled: (data, error) => {
        console.log('Error while setting restaurant payment onboard');
...
export default setRestaurantPaymentOnboard;
