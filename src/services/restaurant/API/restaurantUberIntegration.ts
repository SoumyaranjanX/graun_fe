...
export const postApiRequest = async (payload: any): Promise<ResponseDataType> => {
  const { RESTAURANT_UBER_INTEGRATION } = RESTAURANT_API_URLS;
  return requests.orderPost(RESTAURANT_UBER_INTEGRATION, payload);
};
// NOTE: USE handler when you want to use logic from within the component. else defile the onSuccess, onError etc here
const restaurantUberIntegration = (handler?: HandlerType) => {
...
export default restaurantUberIntegration;
