...
export const postApiRequest = async (payload: any): Promise<ResponseDataType> => {
  const { REMOVE_GRAUNFOOD_INTEGRATION } = RESTAURANT_API_URLS;
  return requests.restaurantPost(REMOVE_GRAUNFOOD_INTEGRATION, payload);
};
// NOTE: USE handler when you want to use logic from within the component. else defile the onSuccess, onError etc here
const removeGraunFoodIntegration = (handler?: HandlerType) => {
...
export default removeGraunFoodIntegration;
